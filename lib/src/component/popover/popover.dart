...
abstract class TuxMenuItemProvider {
...
class TuxMenuItem extends TuxMenuItemProvider {
  Widget image;
  String title;
  var userInfo;
  TextStyle textStyle;
  ...
  @override
  TextStyle get menuTextStyle =>
      textStyle ?? TextStyle(color: TuxColor.basic_200, fontSize: 10.0);

  @override
  TextAlign get menuTextAlign => textAlign ?? TextAlign.center;
}

enum TuxMenuType { big, oneLine }

typedef MenuClickCallback = Function(TuxMenuItemProvider item);
typedef PopupMenuStateChanged = Function(bool isShow);

class TuxPopOverMenu {
  static var itemWidth = 72.0;
  static var itemHeight = 65.0;
  static var arrowHeight = 10.0;
  OverlayEntry _entry;
  List<TuxMenuItemProvider> items;
  ...
  Size _screenSize; // Define ScreenSize of device
  ...
  TuxPopOverMenu(
    ...
    this._backgroundColor = backgroundColor ?? TuxColor.pop_over_color;
    this._lineColor = lineColor ?? TuxColor.pop_over_line;
    this._highlightColor = highlightColor ?? TuxColor.pop_over_color;
    if (context != null) {
      TuxPopOverMenu.context = context;
    }
  }

  void show({Rect rect, GlobalKey widgetKey, List<TuxMenuItemProvider> items}) {
    ...
    this._showRect = rect ?? TuxPopOverMenu.getWidgetGlobalRect(widgetKey);
    this._screenSize = window.physicalSize / window.devicePixelRatio;
    this.dismissCallback = dismissCallback;

    _calculatePosition(TuxPopOverMenu.context);

    _entry = OverlayEntry(builder: (context) {
      return buildPopupMenuLayout(_offset);
    });

    Overlay.of(TuxPopOverMenu.context).insert(_entry);
    _isShow = true;
  ...
  void _calculatePosition(BuildContext context) {
    ...
    _offset = _calculateOffset(TuxPopOverMenu.context);
  }
  ...
  // create line for row items
  List<Widget> _createRows() {
    List<Widget> rows = [];
    for (int i = 0; i < _row; i++) {
      Color color =
          (i < _row - 1 && _row != 1) ? _lineColor : TuxColor.transparent;
  ...
  //Create row for items, row start from 0
  List<Widget> _createRowItems(int row) {
    List<TuxMenuItemProvider> subItems =
  ...
  Widget _createMenuItem(TuxMenuItemProvider item, bool showLine) {
  ...
  void itemClicked(TuxMenuItemProvider item) {
    if (onClickMenu != null) {
      onClickMenu(item);
    }
...
class _MenuItemWidget extends StatefulWidget {
  final TuxMenuItemProvider item;
  final bool showLine;
  final Color lineColor;
  final Color backgroundColor;
  final Color highlightColor;

  final Function(TuxMenuItemProvider item) clickCallback;
...
class _MenuItemWidgetState extends State<_MenuItemWidget> {
  Color highlightColor = TuxColor.pop_over_highlight_color;
  Color color = TuxColor.pop_over_color;
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
          width: TuxPopOverMenu.itemWidth,
          height: TuxPopOverMenu.itemHeight,
          decoration: BoxDecoration(
              color: color,
              border: Border(
                  right: BorderSide(
